...
*
***************************************************************************************/
exports.get_dtlsusrsme = function (req, res) {
    console.log(req.body)
    CafMdl.get_dtlsusrsmeMdl(req.body.data, req.user)
        ...
        .then(function (results) {
			if (results && results.length) {
				for(i=0; i<results.length; i++){
					var adrss =  getEmtpyTxt(results[i].instl_addr1_tx) + getEmtpyTxt(results[i].instl_addr2_tx) + getEmtpyTxt(results[i].instl_lcly_tx) + getEmtpyTxt(results[i].instl_ara_tx) + getEmtpyTxt(results[i].vlge_nm) + getEmtpyTxt(results[i].mndl_nm) + getEmtpyTxt(results[i].dstrt_nm) +  getEmtpyTxt(results[i].instl_std_cd);
					results[i]['adrss'] = adrss;
				}
            }
            df.formatSucessRes(req, res, results, cntxtDtls, '', {});
        }).catch(function (error) {
...
exports.getcafHsiDtls = function (req, res) {
    ...
    /*CafMdl.getcafHsiDtlsMdl(req.params.id, req.user)
        .then(function (results) {
            ...
            var groupBy = 's_no';
            var sortKey = 's_no';
            var groupres = jsonUtils.groupJsonByKey(results, common_feilds, arrFeilds, arrName, groupBy, sortKey, 'desc');
            df.formatSucessRes(req, res, groupres, cntxtDtls, '', {});
        }).catch(function (error) {
            df.formatErrorRes(req, res, error, cntxtDtls, '', {});
        });*/
	CafMdl.getcafhsidatafrchckcafMdl(req.params.id, req.user)
        .then(function (result) {
			CafMdl.getcafHsiDtlsMdl(req.params.id, result[0], req.user)
				.then(function (results) {
					if(result[0].caf_type_id == 1 && result[0].prpd_flag == 1 && result[0].enty_sts_id != 8 && result[0].enty_sts_id != 45){
						var arr = []
						var TU=0;
						var TD=0;
						var TT=0;
						Object.keys(results).forEach(function (key) {
							var dt = new Date();
							var d= new Date(dt.getFullYear(), dt.getMonth()+1, 0).getDate();
							let dayTU = `day_${+key+1}_TU`;
							let dayTD = `day_${+key+1}_TD`;
							arr.push({[dayTU]:results[key].upld_ct})
							arr.push({[dayTD]:results[key].dwnld_ct})
							TU += results[key].upld_ct;
							TD += results[key].dwnld_ct;
							TT += results[key].ttl_ct;
							if (key==results.length -1){
								for ( key; key < d-1; key++) {
									let dayTU = `day_${+key+2}_TU`;
									let dayTD = `day_${+key+2}_TD`;
									arr.push({[dayTU]:0})
									arr.push({[dayTD]:0})
								}
							}
						...
						finalObj = [finalObj]
						console.log("ary obj = ",finalObj);
						var data={};
						data["s_no"]=1
						data["TD"]=TD
						data["TU"]=TU
						data["total"]=TT
						data["yr_ct"]=results[0].yr
						data["mnt_ct"]=results[0].mnth
						data["mnth_usge_lmt_ct"]=results[0].mnth_usge_lmt_ct
						data["eachday"]=finalObj;
...
exports.getAppcafHsiDtls = function (req, res) {
	CafMdl.getcafinfoappMdl(req.params.id).then((cafdata) => {
    ...
    CafMdl.getBoxDtlsMdl(req.params.id, req.user)
        .then(function (results) {
            ...
            if (results && results[0].srl_nu != null && results[0].caf_cnt == 0) {
                ...
                df.formatAppInvalidParamsErrorRes(req, res, results, cntxtDtls, '', { error_status: 500, err_message: "Scanned Box is already in use" });
            }
            else if (results && results[0].srl_nu == null && results[0].caf_cnt == 0) {
                console.log('in step 3');
                df.formatAppInvalidParamsErrorRes(req, res, results, cntxtDtls, '', { error_status: 500, err_message: "Invalid Serial Number" });
            }
            else {
                df.formatAppInvalidParamsErrorRes(req, res, error, cntxtDtls, '', {
                    error_status: 700,
...
exports.getCafBySearchCtrl = function (req, res) {
    ...
    CafMdl.getCafBySearchMdl(check, req.user)
        .then((results) => {
            df.formatSucessRes(req, res, results, cntxtDtls, fnm, {});
        }).catch((error) => {
            df.formatErrorRes(req, res, error, cntxtDtls, fnm, {});
...
exports.getCustomerSlctdSegmntDtaCtrl = function (req, res) {
    ...
    if (req.params.id == 1) {
		...
		CafMdl.getcafinfoappMdl(req.params.cafID).then((cafdata) => {
        CafMdl.getAppcafHsiDtlsMdl(req.params.cafID, req.params.yr, cafdata[0], req.user)
            .then(function (results) {
                ...
                if (results && results.length == 0) {
                    ...
                    _.forIn(_.groupBy(results, 'pckge_id'), (value, key) => {
                        ...
                        pckg_fltr_json.push({
                            ...
                            plan_act: value[0]['plan_act'],
                            plan_exp: value[0]['plan_exp'],
							cycle_strt_dt: value[0]['cycle_strt_dt'],
                            cycle_end_dt:value[0]['cycle_end_dt'],
                            cresrvs: sub_corsrvc_json(value)
                        })
                    })
                    // console.log('pckg_fltr_json --------------------')
...
exports.restartCafCtrl = function (req, res) {
    ...
    log.info(`In ${fnm}`, 0, cntxtDtls);
    var postResultData = [];
    CafMdl.newgetcafdetailsMdl({ id: caf_id }, req.user).then((cafRes) => {
        let restartDta = {
        ...
        extApiCtrl.callApi("BOX RESTART", 1, 15, req.params.caf_id, apiCalls, req.user).then((resp) => {
...
exports.vilidateCAF = function (req, res) {
    ...
    log.info(`In ${fnm}`, 0, cntxtDtls);
    let caf_id = req.params.caf_id;
    request.get(`http://172.17.4.181:4302/dreamops/validate/all/${caf_id}`, function (err, req_res, body) {
        if (err) {
...
exports.getdtlsbyCAFId = function (req, res) {
    ...
    log.info(`In ${fnm}`, 0, cntxtDtls);
    CafMdl.getdtlsbyCAFIdMdl(req.params.caf_id, req.user)
        .then((results) => {
            df.formatSucessRes(req, res, results, cntxtDtls, fnm, {});
        }).catch((error) => {
            df.formatErrorRes(req, res, error, cntxtDtls, fnm, {});
...
exports.updtcafdtlsbyID = function (req, res) {
	...
	/*CafMdl.getdtlsbyCAFIdMdl(id)
        .then((result) => {
			console.log("result in caf updtn",result[0]);
			CafMdl.updtcafdtlsbyIDMdl(data, id, req.user, result[0])
				.then((results) => {
					df.formatSucessRes(req, res, results, cntxtDtls, fnm, {});
				}).catch((error) => {
					df.formatErrorRes(req, res, error, cntxtDtls, fnm, {});
...
exports.spltsupdtnbycafIDCtrl = function (req, res) {
    ...
    var id = req.params.caf_id;
    var data = req.body.data
    CafMdl.updtoldsplitinfoMdl(data.oldonu, id, req.user)
        .then((results) => {
            CafMdl.updtspltbycafId(data, id, req.user).then((cafuptresults) => {

                CafMdl.updtspltinfoMdl(data.splt_id, id, req.user).then((oldspltuptres) => {
...
/**************************************************************************************
* Controller     : newgetcaflmoKycDtlsCtrl
* Parameters     : req,res()
* Description    : get KYC Type details
* Change History :
* 29/01/2020    -  SCRIPT DENERATED  - Initial Function
*
***************************************************************************************/
exports.newgetcaflmoKycDtlsCtrl = (req, res) => {
    var fnm = "getcaflmoKycDtlsCtrl";
    log.info(`In ${fnm}`, 0, cntxtDtls);
    CafMdl.newgetcaflmoKycDtlsMdl(req.body.data, req.user)
        .then((results) => {
            df.formatSucessRes(req, res, results, cntxtDtls, fnm, {});
        }).catch((error) => {
            df.formatErrorRes(req, res, error, cntxtDtls, fnm, {});
...
* Description    : Get Caf Details
* Change History :
* 10-06-2022    -  ramesh  - Initial Function
*
***************************************************************************************/
exports.newuploadCafKycCtrl = function (req, res) {
	const regex = /^[0-9]*$/;
    function img_upload(img, filename, callback) {
        ...
        var dir = '/glits/filestore/uploads/prepaid_caf_reassignment' ;
        console.log(fs.existsSync(dir))
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir);
	...
	/*if (regex.exec(req.body.data.mbl_nu) !== null) {
        if (regex.exec(req.body.data.pincode) !== null) {
            console.log("a number")
            if (req.body.data.pincode.toString().length == 6) {
                if (req.body.data.mbl_nu.toString().length == 10) {*/
	/*CafMdl.checkadhrfrcafmdl(req.body.data, req.user)
                                    .then(function (results) {
										console.log("results",results)
	if (results && results[0].caf_cnt == 0) {*/
		if (req.body.data.kyc_img_tx && req.body.data.kyc_img_tx != undefined && req.body.data.kyc_bck_img_tx && req.body.data.kyc_bck_img_tx != undefined && req.body.data.cstmr_img_tx && req.body.data.cstmr_img_tx != undefined) {
			// attUtil.uploadToS3([req.body.data.kyc_img_tx], 'wetrackon/bss_kyc', (err, attKycChres) => {
				var filename1 = req.body.data.caf_id+'_offr_img_' + Date.now();
				img_upload(req.body.data.kyc_img_tx, filename1, (err, attKycChres) => {
				if (!err) {
					console.log(attKycChres);
					var kycAttchmnt = attKycChres;
					// attUtil.uploadToS3([req.body.data.kyc_bck_img_tx], 'wetrackon/bss_kyc', (err, attKycBckChres) => {
						var filename2 = req.body.data.caf_id+'_offr_img_' + Date.now()+1;
						img_upload(req.body.data.kyc_bck_img_tx, filename2, (err, attKycBckChres) => {
						if (!err) {
							console.log(attKycBckChres);
							var kycBckAttchmnt = attKycBckChres;
							// attUtil.uploadToS3([req.body.data.cstmr_img_tx], 'wetrackon/bss_kyc', (err, attCstmrChres) => {
								var filename3 = req.body.data.caf_id+'_offr_img_' + Date.now()+2;
								img_upload(req.body.data.cstmr_img_tx, filename3, (err, attCstmrChres) => {

								if (!err) {
									console.log(attCstmrChres);
									var cstmrAttchmnt = attCstmrChres;
									console.log('kycAttchmnt \n', kycAttchmnt, 'cstmrAttchmnt \n', cstmrAttchmnt, 'kycBckAttchmnt \n', kycBckAttchmnt);
									 CafMdl.getcafolddata(req.body.data, req.user)
										.then(function (olddata) {
									        CafMdl.newinskycDocLmocafMdl(req.body.data, kycAttchmnt, kycBckAttchmnt, cstmrAttchmnt, olddata[0], req.user)
										        .then(function (results) {
                                                     CafMdl.inschngkycDocLmocafMdl(req.body.data, kycAttchmnt, kycBckAttchmnt, cstmrAttchmnt, olddata[0], req.user)
                                                        .then(function (results) {
                                                             df.formatSucessAppRes(req, res, results, cntxtDtls, '', { error_status: 500, err_message: "KYC upload successfully" });
                                                     })
                                            }).catch(function (error) {
                                                df.formatAppNewInvalidParamsErrorRes(req, res, error, cntxtDtls, '', { error_status: 500, err_message: "KYC upload failed" });
                                            });
                                        })
								} else {
											let error = "Please check image";
											df.formatAppNewInvalidParamsErrorRes(req, res, error, cntxtDtls, '', { error_status: 500, err_message: "KYC upload failed" });
										}
							})
						} else {
	...
	/*} else if (results &&  results[0].caf_cnt > 0) {
		console.log('in step 2');
		df.formatErrorRes(req, res, results, cntxtDtls, '', { error_status: 500, err_message: "Aadhaar Number is already Updated Once" });
	} else {
		let error = "No Data Avilable"
		df.formatErrorRes(req, res, error, cntxtDtls, '', {
			error_status: 700,
			err_message: "Database error"
		});
	...
	/*} else {
                    console.log("not a length number");
                    df.formatErrorRes(req, res, results, cntxtDtls, '', { error_status: 500, err_message: "Please check Mobile Number Length Must Be 10" });
                }
            } else {
                console.log("not a length number");
                df.formatErrorRes(req, res, results, cntxtDtls, '', { error_status: 500, err_message: "Please check Pincode Length Must Be 6" });
            }
        } else {
            console.log("not a number");
            df.formatErrorRes(req, res, results, cntxtDtls, '', { error_status: 500, err_message: "Please check  Pincode special characters are not allowed" });
        }
    } else {
        console.log("not a number");
        df.formatErrorRes(req, res, results, cntxtDtls, '', { error_status: 500, err_message: "Please check Mobile Number  special characters are not allowed" });
    }*/
}
...
/**************************************************************************************
* Controller     : sbscruploadCafKycCtrl
* Parameters     : req,res()
* Description    : Get Caf Details
* Change History :
* 19-07-2022    -  Ramesh Patlola  - Initial Function
*
***************************************************************************************/
exports.sbscruploadCafKycCtrl = function (req, res) {
    console.log("req.user", req.body.data.cstmr_id)
    console.log("req.user", req.body.data.trms_cndtn_in);
    const regex = /^[0-9]*$/;
    function img_upload(img, filename, callback) {
        var matches = img.match(/^data:([A-Za-z-+/]+);base64,(.+)$/),
            response = {};
        ...
        let type = decodedImg.type;
        let extension = mime.extension(type);
        var name = req.body.data.caf_id + '_offr_img_' + Date.now();
        let fileName = filename + "." + extension;
        var dir = '/glits/filestore/uploads/caf_reassignment';
        console.log(fs.existsSync(dir))
        if (!fs.existsSync(dir)) {
                                    ...
                                    if (!err) {
                                        ...
                                        img_upload(req.body.data.cstmr_img_tx, filename3, (err, attCstmrChres) => {
                                            ...
                                            if (!err) {
                                                ...
                                                var cstmrAttchmnt = attCstmrChres;
                                                console.log('kycAttchmnt \n', kycAttchmnt, 'cstmrAttchmnt \n', cstmrAttchmnt, 'kycBckAttchmnt \n', kycBckAttchmnt);
                                                CafMdl.sbscrinskycDocLmocafMdl(req.body.data, kycAttchmnt, kycBckAttchmnt, cstmrAttchmnt, req.user)
                                                    .then(function (results) {
                                                        df.formatSucessRes(req, res, results, cntxtDtls, '', {});
                                                    }).catch(function (error) {
                                                        df.formatErrorRes(req, res, error, cntxtDtls, '', {});
...
*
***************************************************************************************/
exports.newuploadSelfCafKyc = function (req, res) {
    ...
    function img_upload(img, flname, callback) {
        ...
        var dir = '/glits/filestore/uploads/prepaidkyc' ;
        console.log(fs.existsSync(dir))
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir);
	    ...
	    CafMdl.checkadhrfrselflmomdl(req.body.data, req.user)
                                    .then(function (results) {
										console.log("results",results)
										console.log("results",results[0].caf_cnt)
										console.log("results",results[0].agnt_cd)
	if ( results[0].caf_cnt == 0) {
		 if (req.body.data.kyc_img_tx && req.body.data.kyc_img_tx != undefined && req.body.data.kyc_bck_img_tx && req.body.data.kyc_bck_img_tx != undefined && req.body.data.cstmr_img_tx && req.body.data.cstmr_img_tx != undefined) {
		//     attUtil.uploadToS3([req.body.data.kyc_img_tx], 'wetrackon/bss_kyc     ', (err, attKycChres) => {
			let fle1name = req.user.lmo_cd + '_offr_img_' + Date.now();
		img_upload(req.body.data.kyc_img_tx, fle1name,  (err, attKycChres) => {
			if (!err) {
				console.log(attKycChres);
				var kycAttchmnt = attKycChres;
				//             attUtil.uploadToS3([req.body.data.kyc_bck_img_tx], 'wetrackon/bss_kyc', (err, attKycBckChres) => {
					let fle2name = req.user.lmo_cd + '_offr_img_' + Date.now()+1;
				img_upload(req.body.data.kyc_bck_img_tx, fle2name, (err, attKycBckChres) => {
					if (!err) {
						console.log(attKycBckChres);
						var kycBckAttchmnt = attKycBckChres;
						//                     attUtil.uploadToS3([req.body.data.cstmr_img_tx], 'wetrackon/bss_kyc', (err, attCstmrChres) => {
							let fle3name = req.user.lmo_cd + '_offr_img_' + Date.now()+2;
						img_upload(req.body.data.cstmr_img_tx, fle3name, (err, attCstmrChres) => {
							if (!err) {
								console.log(attCstmrChres);
								var cstmrAttchmnt = attCstmrChres;
								let fle4name = req.user.lmo_cd + '_offr_img_' + Date.now()+3;
								img_upload(req.body.data.kyc_canch_img_tx, fle4name, (err, cancelchqCstmr) => {
									if (!err) {
										console.log(cancelchqCstmr);
										var cancelchqCstmrAttchmnt = cancelchqCstmr;
										let fle5name = req.user.lmo_cd + '_offr_img_' + Date.now()+4;
										img_upload(req.body.data.kyc_incop_img_tx, fle5name, (err, incorparationCstmr) => {
											if (!err) {
												console.log(incorparationCstmr);
												var incorparationCstmrAttchmnt = incorparationCstmr;
												let fle6name = req.user.lmo_cd + '_offr_img_' + Date.now()+5;
												img_upload(req.body.data.kyc_dpan_img_tx, fle6name, (err, panCstmr) => {
													if (!err) {
														console.log(panCstmr);
														var panCstmrAttchmnt = panCstmr;
														let fle7name = req.user.lmo_cd + '_offr_img_' + Date.now()+6;
														img_upload(req.body.data.kyc_didpf_img_tx, fle7name, (err, idproofCstmr) => {
															if (!err) {
																console.log(idproofCstmr);
																var idproofCstmrAttchmnt = idproofCstmr;
																console.log('kycAttchmnt \n', kycAttchmnt, 'cstmrAttchmnt \n', cstmrAttchmnt, 'kycBckAttchmnt \n', kycBckAttchmnt);
																CafMdl.newinsSelfkycDocLmocafMdl(req.body.data, kycAttchmnt, kycBckAttchmnt, cstmrAttchmnt, cancelchqCstmrAttchmnt, incorparationCstmrAttchmnt, panCstmrAttchmnt, idproofCstmrAttchmnt, req.user)
																	.then(function (results) {
																		df.formatSucessAppRes(req, res, results, cntxtDtls, '', { error_status: 500, err_message: "KYC upload success" });
																	}).catch(function (error) {
																		df.formatAppNewInvalidParamsErrorRes(req, res, error, cntxtDtls, '', { error_status: 500, err_message: "KYC upload failed" });
																	});
																} else {
	...
	} else if ( results[0].caf_cnt > 0) {
				console.log('in step 2');
				df.formatSucessAppRes(req, res, results, cntxtDtls, '', { status: 202, message: "Already KYC Completed, Thank You : "+ results[0].agnt_cd});
	} else {
			let error = "No Data Avilable"
			df.formatErrorRes(req, res, error, cntxtDtls, '', {
				error_status: 700,
...
*
***************************************************************************************/
exports.get_cafdtlsfrIms = function (req, res) {
    console.log(JSON.stringify(req.user))
    CafMdl.get_cafdtlsfrImsMdl(req.body.data, req.user)
        .then(function (results) {
            df.formatSucessRes(req, res, results, cntxtDtls, '', {});
        }).catch(function (error) {
            df.formatErrorRes(req, res, error, cntxtDtls, '', {});
...
*
***************************************************************************************/
exports.get_cafdtlscmplnts = function (req, res) {
    console.log(JSON.stringify(req.user))
    CafMdl.get_cafdtlscmplntsMdl(req.body.data, req.user)
        .then(function (results) {
			console.log("result for caf data", results);
            df.formatSucessRes(req, res, results, cntxtDtls, '', {});
        }).catch(function (error) {
            df.formatErrorRes(req, res, error, cntxtDtls, '', {});
        }); 
...
/**************************************************************************************
* Controller     : getcafaddoninvceDtls
* Parameters     : req,res()
* Description    : Get Caf Details
* Change History :
* 18-01-2022    -  Ramesh  - Initial Function
*
***************************************************************************************/
exports.getcafaddoninvceDtls = function (req, res) {

	console.log("request from add on invoice",req.params.id);
    CafMdl.getcafaddoninvceDtlsMdl(req.params.id, req.user)
        .then(function (results) {
			console.log("results",results);
            var common_feilds = ['s_no','invce_yr', 'caf_invce_id','invce_mm',  'pckage_type_nm', 'chrge_cde_dscn_tx'];
            var arrFeilds = ['pckge_nm', 'pckage_type_nm', 'chrge_cde_dscn_tx', 'chrge_at', 'tax_at','tl_at'];
            var arrName = 'eachday';
            var groupBy = 'invce_mm';
            var sortKey = 's_no';
            var groupres = jsonUtils.groupJsonByKey(results, common_feilds, arrFeilds, arrName, groupBy, sortKey, 'desc');
...
/**************************************************************************************
* Controller     : getcafaddonsInvoiceChargesDtls
* Parameters     : req,res()
* Description    : Get Caf Invoice Cbarges Details
* Change History :
* 21-01-2022    -  ramesh  - Initial Function
*
***************************************************************************************/
exports.getcafaddonsInvoiceChargesDtls = function (req, res) {

    console.log(' ---------------------------------------------- getcafaddonsInvoiceChargesDtls', req.params.id)
    CafMdl.getcafaddonsInvoiceChargesDtlsMdl(req.params.id, req.user)
        .then(function (results) {
            var ttlamt = 0.00;
            var count = 0;
            for(let i=0; i < results.length; i++){
                ttlamt += parseFloat(results[i].tl_at);
                count ++;
            }
            if ( count == results.length){
                for(let i=0; i<results.length; i++){
                    results[i]['ttlamt'] = ttlamt
                }
                df.formatSucessRes(req, res, results, cntxtDtls, '', {});
            }
        }).catch(function (error) {
...
*
***************************************************************************************/
exports.getdtcountfrcmplntsCtrl = function (req, res) {
    console.log(JSON.stringify(req.user))
    CafMdl.getdtcountfrcmplntsMdl(req.body.data, req.user)
        .then(function (results) {
			console.log("result for caf data", results);
            df.formatSucessRes(req, res, results, cntxtDtls, '', {});
        }).catch(function (error) {
...
/**************************************************************************************
* Controller     : getbbnlcafdtls
* Parameters     : req,res()
* Description    : Add new  CafStatus
* Change History :
* 03/09/2023    -  Ramesh  - Initial Function
*
***************************************************************************************/
exports.getbbnlcafdtls = function (req, res) {
    console.log(JSON.stringify(req.user))
    CafMdl.getbbnlcafdtlMdl(req.body.data, req.user)
        .then(function (results) {
            if (results && results.length) {
                for (i = 0; i < results.length; i++) {
                    var adrss = getEmtpyTxt(results[i].instl_addr1_tx) + getEmtpyTxt(results[i].instl_addr2_tx) + getEmtpyTxt(results[i].instl_lcly_tx) + getEmtpyTxt(results[i].instl_ara_tx) + getEmtpyTxt(results[i].vlge_nm) + getEmtpyTxt(results[i].mndl_nm) + getEmtpyTxt(results[i].dstrt_nm) + getEmtpyTxt(results[i].instl_std_cd);
...
/**************************************************************************************
* Controller     : getgoicafdtlCtrl
* Parameters     : req,res()
* Description    : Add new  CafStatus
* Change History :
* 13/11/2023    -  Ramesh  - Initial Function
*
***************************************************************************************/
exports.getgoicafdtlCtrl = function (req, res) {
    console.log(JSON.stringify(req.user))
    CafMdl.getgoicafdtlMdl(req.body.data, req.user)
        .then(function (results) {
            if (results && results.length) {
                for (i = 0; i < results.length; i++) {
                    var adrss = getEmtpyTxt(results[i].instl_addr1_tx) + getEmtpyTxt(results[i].instl_addr2_tx) + getEmtpyTxt(results[i].instl_lcly_tx) + getEmtpyTxt(results[i].instl_ara_tx) + getEmtpyTxt(results[i].vlge_nm) + getEmtpyTxt(results[i].mndl_nm) + getEmtpyTxt(results[i].dstrt_nm) + getEmtpyTxt(results[i].instl_std_cd);
...
/**************************************************************************************
* Controller     : getentcafdetailsCtrl
* Parameters     : req,res()
* Description    : 
*
***************************************************************************************/
exports.getentcafdetailsCtrl = function (req, res) {
    var fnm = "getentcafdetailsCtrl";
    log.info(`In ${fnm}`, 0, cntxtDtls);
    // 
    CafMdl.getentcafdetailsMdl(req.params.caf_id, req.user)
        .then((results) => {
            df.formatSucessRes(req, res, results, cntxtDtls, fnm, {});
        }).catch((error) => {
            df.formatErrorRes(req, res, error, cntxtDtls, fnm, {});
...
/**************************************************************************************
* Controller     : getapsflBbnlPopDtls
* Parameters     : req,res()
* Description    : Add new  CafStatus
* Change History :
* 03/02/2020    -  Ganesh  - Initial Function
*
***************************************************************************************/
exports.getapsflBbnlPopDtls = function (req, res) {

    CafMdl.getapsflBbnlPopDtls(req.params.id, req.params.apsfBbnl, req.user)
        .then(function (results) {
            df.formatSucessRes(req, res, results, cntxtDtls, '', {});
        }).catch(function (error) {
            df.formatErrorRes(req, res, error, cntxtDtls, '', {});
...
/**************************************************************************************
* Controller     : getLmolisrforMSO
* Parameters     : req,res()
* Description    : get KYC Type details
* Change History : apiv2
* 03/02/2024    -  Ramesh Patlola  - Initial Function
*
***************************************************************************************/
exports.getLmolisrforMSOCtrl = (req, res) => {
    var fnm = "getcaflmoKycDtlsCtrl";
    log.info(`In ${fnm}`, 0, cntxtDtls);
    CafMdl.getLmolisrforMSOMdl(req.body, req.user)
        .then((results) => {
            df.formatSucessRes(req, res, results, cntxtDtls, fnm, {});
        }).catch((error) => {
            df.formatErrorRes(req, res, error, cntxtDtls, fnm, {});
