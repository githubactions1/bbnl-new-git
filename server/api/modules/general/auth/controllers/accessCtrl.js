...
exports.hasToken = (req, res, next) => {
    ...
    //             token = jwt_tkn; //JWT token
			...
			if (user.mrcht_usr_id == 26263 || user.mrcht_usr_id == 16235 || user.mrcht_usr_id == 25930 || user.mrcht_usr_id == 15291 || user.mrcht_usr_id == 27182 || user.mrcht_usr_id == 13899 || user.mrcht_usr_id == 21010){
                validSeconds = 5;
                ignoreExp = false;
            }
            // //console.log('*************', validateToken.isExpired(token, validSeconds, user));
            if (validateToken.isExpired(token, validSeconds, user)) {
                const accessToken = validateToken.generateNewToken(token, 'glitssunil', ignoreExp);
                ...
                //console.log("in else")
                var verified;
                var verifyOptions = { audience: 'http://glits.com', subject: user.mrcht_usr_id + '_user', algorithm: ["HS256"] };
                //console.log(verifyOptions)
                try {
                    verified = jwt.verify(token, 'glitssunil', verifyOptions);
                }
                catch (err) {
                    verified = null;
                }
