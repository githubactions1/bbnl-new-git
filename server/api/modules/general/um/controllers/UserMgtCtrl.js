    ...
    usrmngtmdl.checkAgntMdl(req.body.data, req.user)
        .then((results) => {
            ...
            if (results.length) {
				...
				var message = `Your+One+Time+Password+is+${data.code}+Please+use+this+OTP+to+validate+your+login+-Team+APSFL`;

                mailUtls.sendDSMS(req.body.data.agnt_mbl_nu, message, '1107161746884722114', function (err, response) {
                    if (err) {
                        return df.formatErrorRes(res, error, cntxtDtls, fnm, {});
                    }
                    // data.uuid = response.uuid;
                    // Model gets called Here
                    usrmngtmdl.sendOtpMdl(data, req.user)
                        .then(function (results) {
                            df.formatSucessRes(req, res, results, cntxtDtls, fnm, {});
                        })
                        .catch(function (error) {
...
exports.postAgntVldtOtpCtrl = function (req, res) {
    ...
    if (req.body.data.phno && req.body.data.otp) {
        usrmngtmdl.postAgntVldtOtpMdl(req.body.data, req.user).then((results) => {
            if (results.length) {
...
exports.generateXmlCtrl = (req, res) => {
    ...
    usrmngtmdl.checkcaf(req.params.id, req.user).then((results)=>{
        if (results.length) {
            usrmngtmdl.cafdtlsgetMdl(req.params.id, req.user).then((results1) => {
                console.log(results1.length);
                console.log("result for ZTC file ", results1);
                console.log("result for ZTC file flag ", results1[0]['flag']);
                if (results1.length>0) {
					if ( results1[0]['flag'] == 2 ) {
						let cafdata = []
                    _.forIn(_.groupBy(results1, 'cpeslno'), (value, key) => {
                        if (value.length > 1) {
                            cafdata.push({
                                ...
                                username: value[0].Huaweiusername,
								flag: value[0].flag,
                                //phoneno1: "0000",
                                //passwrd1: '',
                                //username1: '',
                                ...
                                username1: value[1].Huaweiusername,
...
exports.generateXmlMultiCtrl = (req, res) => {
    ...
    usrmngtmdl.cafdtlsMultiMdl(req.params.frmdt, req.params.todt, req.user).then((results1) => {
        if (results1) {
            console.log("caf_dat")
			if ( results1[0]['flag'] == 2 ){
				let cafdata = []
...
exports.getLMOContacts = function (req, res) {
    ...
    log.info(`In ${fnm}`, 0, cntxtDtls);
    console.log(req.body.data);
    usrmngtmdl.getLMOContactsMdl(req.params.agent_id, req.user).then((results) => {
        let fltr_json = []
        if(results.length > 0)
        {
