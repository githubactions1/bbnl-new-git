...
@Component({
  selector: 'app-mso-enrollment',
  templateUrl: './mso-enrollment.component.html',
  styleUrls: ['./mso-enrollment.component.scss']
})

export class MsoEnrollmentComponent implements OnInit {
  ...
  getHeaderDtls = function () { return { "title": "Tenant Details", "icon": "people_outline" } };
  ...
  constructor(private _fuseSidebarService: DsSidebarService, public fb: FormBuilder, public crdsrv: CrudService, public datePipe: DatePipe,
    private _snackBar: MatSnackBar, public dialog: MatDialog, private cd: ChangeDetectorRef, private route: ActivatedRoute, private router: Router,private _dsConfigService: DsConfigService,
...
//     });
    ...
    let gstNumber = /^([0]{1}[1-9]{1}|[1-2]{1}[0-9]{1}|[3]{1}[0-7]{1})([a-zA-Z]{5}[0-9]{4}[a-zA-Z]{1}[1-9a-zA-Z]{1}[zZ]{1}[0-9a-zA-Z]{1})+$/;
    let adharnumber = /^\d{12}$/;
    ...
    this.agntFormGroup = this.fb.group({
      ...
      orgn_typ: new FormControl('', Validators.required),
      ...
      officeAddress: this.fb.group({
        ...
        ofce_State: new FormControl('', Validators.required),
  ...
  getOrgnTypeLst(){
    const rte = `agent/organization/types`;
    this.crdsrv.get(rte).subscribe((res) => {
      if (res['status'] == 200) {
        this.orgnTypLst = res['data'];
      }
    });
  }
  ...
  addSrvngAreas() {
    // this.selectedSrvAraSte = '';
    // this.selectedSrvAraDsrt = '';
    // this.selectedSrvAraMndl = '';
    
    this.srvngAreas.push(this.fb.group({
      ara_nm: '', cbl_type: '', rng_cbl_lngth: '', srv_ara_state: '', srv_ara_dstrt: '', srv_ara_mndl: '', srv_ara_vlge: '',
      sbsc_ct: '', cnct_ct: '', dgtl_cnct_ct: '', anlg_cnct_ct: '', selectedSrvAraSte: '', selectedSrvAraDsrt: '',
      selectedSrvAraMndl: '', selectedSrvAraVlge: ''
    }));
  }

  deleteSrvngAreas(index) {
  ...
  onValueChanges(): void {
    this.agntFormGroup.get('officeAddress').valueChanges.subscribe(val => {
      if (this.isChecked) {
        ...
        this.agntFormGroup.get('branchAddress').get('brnch_address1').setValue(this.agntFormGroup.get('officeAddress').get('ofce_address1').value);
        this.agntFormGroup.get('branchAddress').get('brnch_address2').setValue(this.agntFormGroup.get('officeAddress').get('ofce_address2').value);
        this.agntFormGroup.get('branchAddress').get('brnch_lcty_nm').setValue(this.agntFormGroup.get('officeAddress').get('ofce_lcty_nm').value);
        ...
        this.agntFormGroup.get('branchAddress').get('brnch_lndline_cd').setValue(this.agntFormGroup.get('officeAddress').get('ofce_lndline_cd').value);
  ...
  getTntDtls() {
    ...
    this.agntFormGroup.get('orgn_typ').setValue('');
    ...
    this.agntFormGroup.get('ofce_address1').setValue('');
    this.agntFormGroup.get('ofce_address2').setValue('');
    this.agntFormGroup.get('ofce_lcty_nm').setValue('');
    ...
    this.agntFormGroup.get('ofce_lndline_cd').setValue('');
    ...
    this.agntFormGroup.get('brnch_address1').setValue('');
    this.agntFormGroup.get('brnch_address2').setValue('');
    this.agntFormGroup.get('brnch_lcty_nm').setValue('');
    ...
    this.agntFormGroup.get('brnch_lndline_cd').setValue('');
  ...
  checkValue(e) {
    if (e.checked == true) {
      ...
      let data = this.agntFormGroup.value.officeAddress;
      // get('officeAddress').value;
  ...
  saveAgent() {
    ...
    const agntData = {
      agntInfo: {
        agnt_Typ: 3,
        orgn_typ: this.agntFormGroup.value.orgn_typ,
    ...
    for (let k = 0; k < this.agntFormGroup.value.sb_stns.length; k++) {
      if (this.agntFormGroup.value.sb_stns[k].sb_stn_dst != null) {
    ...
    // for (let m = 0; m < fnlAgntDtls.length; m++){
    //   if (fnlAgntDtls[m].officeAddress){
    //     this.officeAddress = fnlAgntDtls[m].officeAddress;
    //   }
    //   if (fnlAgntDtls[m].branchAddress){
    //     this.branchAddress = fnlAgntDtls[m].branchAddress;
    //   }
    //   if (fnlAgntDtls[m].agntInfo){
    //     this.agntInfo = fnlAgntDtls[m].agntInfo;
    //   }
    // }
    // const fnlAgntDtls = { 
    //   officeAddress: this.officeAddress,
    //   branchAddress: this.branchAddress,
    //   agntInfo: this.agntInfo
    // };
    // return;
    let data = {
    ...
    checkDtls['ctgry_id'] = 3;
    console.log(checkDtls);
    // return;
  ...
  getAgntDtls(): any{
    ...
    this.crdsrv.create(getAgntDta, rte).subscribe((res) => {
      ...
      if (agnt_info_dtls == undefined) {
        this.shwRegMsg = "Details not available for the respective Aadhar & mobile number,Please fill the form to register as MSO";
        ...
        this.shwRegMsg = "This MSO is already registered but pending for approval, Please contact APSFL team to know the status";
        this.selfRegPndg = false;
        this.sbmtBtnDsble = true;
      } else if (agnt_info_dtls.slf_rgnrn_ts != null && agnt_info_dtls.onbrd_in == 1){
        this.shwRegMsg = "This MSO is already registered and approved,please login with your credentails that are send to your email";
        this.selfRegPndg = false;
        this.sbmtBtnDsble = true;
