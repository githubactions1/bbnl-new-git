...
@Component({
  selector: 'app-goi-ent-caf-form',
  templateUrl: './goi-ent-caf-form.component.html',
  styleUrls: ['./goi-ent-caf-form.component.scss']
})
export class GoiEntCafFormComponent implements OnInit {
  ...
  getHeaderDtls = function () { return { "title": 'GOI Add New CAF', "icon": "people_outline" } }
  blng_frqncy_lst: any;
  mndal_lst: any;
  mandalId: any;
  vilge_lst: any;
  ...
  constructor(private _dsSidebarService: DsSidebarService, private http: HttpClient, private router: Router, private _formBuilder: FormBuilder, private crdsrv: CrudService, private datePipe: DatePipe, private snackBar: MatSnackBar, public dialog: MatDialog, public TransfereService: TransfereService) {
    ...
    const permTxt = 'GOI Enterprise CAF Creation';
  ...
  ngOnInit() {
    ...
    if (this.frmData == undefined) {
      ...
      this.getHeaderDtls = function () { return { "title": 'BBNL Add New CAF', "icon": "people_outline" } }
    }
  ...
  onretrive() {
    ...
    this.http.get<any>('/caf/aadhaar/' + this.firstFormGroup.value.custInfo.adhr_nu).subscribe(data => {
      this.loader = false;
      console.log(data);
      this.aadhaar = data.data.res[0];
      console.log(this.aadhaar);
      console.log("aadhaar", this.aadhaar.GENDER)
      if (data['status'] == "202") {
  ...
  UpdateFrmdata() {
    ...
    data["gp_code"] = this.firstFormGroup.value.custInfo.gp_code
    console.log(data)
  ...
  Viewpoplst() {
    ...
    const rte = `caf/getPopByAgntId/${this.usrdtls.usr_ctgry_ky}/3`;
    this.crdsrv.get(rte).subscribe((res) => {
  ...
  getpopLst() {
    console.log("pop")
    if (this.frm_actn == "update") {
      const rte = `caf/getPopByAgntId/${this.frmData.agnt_id}/3`;
      this.crdsrv.get(rte).subscribe((res) => {
        this.poplst = res['data'];
        console.log(res['data'])
        console.log(this.poplst)
    ...
    this.crdsrv.get(rte).subscribe((res) => {
      ...
      if (add_typ == 1) {
        ...
        for (let i = 0; i < this.instl_mndl_lst.length; i++) {
          ...
          // console.log(str)
          if (this.aadhaar.MANDAL_NAME.toLowerCase() == str.toLowerCase()) {
            console.log("hai")
  ...
  newentry() {
    ...
    data["caf_type_id"] = 2
    data['link_typ'] = this.firstFormGroup.value.custInfo.link_typ
    data['org_typ'] = this.firstFormGroup.value.custInfo.org_typ
    data["apsfl_bbnl"] = 5
