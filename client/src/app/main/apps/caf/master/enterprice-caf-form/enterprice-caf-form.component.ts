...
@Component({
  selector: 'app-enterprice-caf-form',
  templateUrl: './enterprice-caf-form.component.html',
  styleUrls: ['./enterprice-caf-form.component.scss']
})
export class EnterpriceCafFormComponent implements OnInit {
  pckgeProperties: any
  distId :any;
  ...
  ngOnInit() {
    ...
    this.firstFormGroup = this._formBuilder.group({
      ...
      cntprsnName: ['', [Validators, Validators.pattern(namePattern)]],
        ...
        adhr_nu:   ['',[Validators.pattern(adharnumber),  Validators]],
        pan_nu: [''],
    ...
    this.getGender();
	  ...
	  //this.firstFormGroup.get('custInfo').get('loc_vlge_id').valueChanges.subscribe(value => {
        //this.getvlg(value)
      //});
  }
  get customerInfo() {
  ...
  gettypeillibb() {
    const rte = `caf/EntrpeCstmrillibbTyp`;
    this.crdsrv.get(rte).subscribe((res) => {
      this.typeillibb = res['data'];
      console.log(this.typeillibb)
    });
  }
  getsuborg() {
...
this.nxt=false
  ...
  checkValue(event: any) {
    ...
    if (event.checked == true) {
      ...
      console.log(data)
      this.firstFormGroup.get('custInfo').get('blng_cntct_nm').setValue(data.cntprsnName);
  ...
  newentry() {
    ...
    this.firstFormGroup.value['actvtn_dt'] = this.datePipe.transform(this.firstFormGroup.value.actvn_dt, 'yyyy-MM-dd');

    // this.usrdtls = JSON.parse(localStorage.getItem('usrDtls'));
    // const rte = "caf/insrtentcaf";
    ...
    data["blng_frqny_id"] =this.firstFormGroup.value.custInfo.frqncy_id
    data["gender"] = this.firstFormGroup.value.custInfo.gndr_id
    data["cstmr_emle_tx"] = this.firstFormGroup.value.custInfo.loc_eml1_tx
    data["blng_dstrt_id"]=this.firstFormGroup.value.custInfo.blng_dstrct_id
    data["blng_mndle_id"]=this.firstFormGroup.value.custInfo.blng_mndl_id
    data["blng_cty_id"]=this.firstFormGroup.value.custInfo.blng_vlge_id
    ...
    data["blng_lclty_tx"]=this.firstFormGroup.value.custInfo.blng_lcly_tx
    data["actvtn_dt"] = this.datePipe.transform(this.firstFormGroup.value.actvn_dt, 'yyyy-MM-dd');
    ...
    data["frst_nm"] = data.orgName
    data["lst_nm"] = data.cntprsnName
   data["mdlr_nm"] =" "
    console.log(this.pckgeProperties)
