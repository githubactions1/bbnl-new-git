...
@Component({
  selector: 'app-marketing-operational',
  templateUrl: './marketing-operational.component.html',
  styleUrls: ['./marketing-operational.component.scss']
})
export class MarketingOperationalComponent implements OnInit {
  showLdr: boolean;
  stpBxsCount: any;
  agntCount: any;
  cafCount: any;
...
getCard4(){
  ...
  chequegetlinkUrl(id){
      if(this.lmokycdetails && this.lmokycdetails.length > 0){
        if(id==1){
          window.open(`https://bbnlbss.apsfl.co.in/prepaidimages${this.lmocnclchck}`)
        } else if(id==2){
          window.open(`https://bbnlbss.apsfl.co.in/prepaidimages${this.lmoaadhar}`)
        } else if(id==3){
          window.open(`https://bbnlbss.apsfl.co.in/prepaidimages${this.lmopan}`)
        } else if(id==4){
          window.open(`https://bbnlbss.apsfl.co.in/prepaidimages${this.lmoidproof}`)
        } else if(id==5){
          window.open(`https://bbnlbss.apsfl.co.in/prepaidimages${this.lmocertificate}`)
        }
      } else {
        this._snackBar.open('No Data Found in DB... Please Inform LMO To Update The KYC', '', {
          duration: 2000,
