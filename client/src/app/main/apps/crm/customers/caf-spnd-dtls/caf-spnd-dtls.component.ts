...
@Component({
  selector: 'app-caf-spnd-dtls',
  templateUrl: './caf-spnd-dtls.component.html',
  styleUrls: ['./caf-spnd-dtls.component.scss']
})
export class CafSpndDtlsComponent implements OnInit {
  lmocd: any;
  agnt_id: any;
  permissions;
  agntCafLst: any;
  ...
  shwPermMsg: string;
  getHeaderDtls = function (): any { return { 'title': 'Caf Suspend And Resume Details', 'icon': 'list_alt' }; };
  ...
  constructor(private _dsSidebarService: DsSidebarService, public crdSrv: CrudService, public atmSrv: AtomService,
    ...
    const permTxt = 'caf spnd rsme Details';
  ...
  getagntMdfcnCafsData(data): any {
    ...
    this.crdSrv.create(data, rte).subscribe((res) => {
      ...
      this.agntTrmndCafLstColumnDefs = [
        { headerName: 'S.No', field: 'sno', alignment: 'center', cellClass: 'pm-grid-number-cell', width: 200, height: 40, search: false, filter: false },
        { headerName: 'Suspend Dates', field: 'spnd_dt', alignment: 'center', cellClass: 'pm-grid-number-cell', width: 400, height: 40, filter: true },
        { headerName: 'Resume Dates', field: 'rsme_dt', alignment: 'center', cellClass: 'pm-grid-number-cell', width: 400, height: 40, filter: true },
        { headerName: 'Termination Date', field: 'trmnd_dt== null ?"":trmnd_dt', alignment: 'center', cellClass: 'pm-grid-number-cell', width: 300, height: 40, filter: true },
      ];
    },
    (err) => {
      console.log("check the caf u submitted ")
