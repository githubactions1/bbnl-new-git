...
export class ViewComplaintsComponent implements OnInit {
  ...
  getHeaderDtls = function (): any { return { 'title': "View Complaint List", 'icon': 'people_outline' }; };
  msoLmoDtls: any;
  complaintsview: any;
  ...
  getDtls(){
    this.loading = true;
    const cmprte = `subscriberApp/OCCgetcafinsrtcmplnt/${this.caf_num}`;
    this.crdsrv.get(cmprte).subscribe((res) => {
      if (res['status'] === 200) {
        ...
        this.columnDefs = [
          ...
          { headerName: 'District Name', field: 'dstrt_nm', alignment: 'center', cellClass: 'pm-grid-number-cell', width: 150, dataType: 'number', filter: true },
  ...
  getcomplaintLst(): any{
    this.loading = true;
    const cmprte = `subscriberApp/OCCgetinsrtcmplnt`;
    this.crdsrv.get(cmprte).subscribe((res) => {
      if (res['status'] === 200) {
        this.loading = false;
        this.rowData = res['data'];
