...
@Component({
  selector: 'app-lmo-wallet-list',
  templateUrl: './lmo-wallet-list.component.html',
  styleUrls: ['./lmo-wallet-list.component.scss']
})
export class LmoWalletListComponent implements OnInit {
    ...
    getHeaderDtls = function () {
        return { title: "APSFL - LMO Wallet List", icon: "people_outline" };
    };
    ...
    if (this.walletreportsData.invalid) {
      ...
      if(this.usrdtls.usr_ctgry_id == 8){
        if (remaining_days < 93) {
          ...
          this.crdsrv.create(data, "lmoprepaid/reportlist").subscribe((res) => {
