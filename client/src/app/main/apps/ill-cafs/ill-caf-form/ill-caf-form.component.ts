...
@Component({
  selector: 'app-ill-caf-form',
  templateUrl: './ill-caf-form.component.html', 
  styleUrls: ['./ill-caf-form.component.scss']
})
export class IllCafFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.firstFormGroup = this._formBuilder.group({
      custInfo: this._formBuilder.group({
        ...
        custmrTyp: [''],
        tle_nm: [''],
        hdOrgName:[{ value: this.entfrmData[0].entfrmData.cstmr_nm, disabled: true }, Validators.required],
        slctdpckg:[{ value: this.entfrmData[0].ill_pckge_nm, disabled: true }, Validators.required],
        dob_dt: ['', Validators.required],
        rltve_nm: ['', [Validators, Validators.pattern(namePattern)]],
        pan_nu: [''],
        loc_eml1_tx: [''],
        lsd_ln:['', Validators.required],
        loc_lmdle1_nu: ['', [Validators.required, Validators.pattern(phoneNumber)]],
        frqncy_id: ['', Validators.required],
        instd_by_prntnr_id: ['', Validators.required],
        prtnr_mntce_chrge_at: ['', Validators.required],
    ...
    // this.firstFormGroup.get('custInfo').get('hdOrgName').setValue(this.entfrmData.cstmr_nm)
...
console.log(this.firstFormGroup.get('custInfo').get('adhr_nu'))
  ...
  //  this.getStgetpackagesates();
    ...
    if (this.frmData == undefined) {
      ...
      this.getHeaderDtls = function () { return { "title": 'Add New ill CAF', "icon": "people_outline" } }
    }
  ...
  slctdsrvc(){
    console.log(this.firstFormGroup.get('custInfo').get('lsd_ln'));
  }
  onpartner(id){
    console.log(id)
    if(id==4){
      this.firstFormGroup.get('custInfo').get('prtnr_mntce_chrge_at').setValue(0);
      this.read = true
    }
    else{
      this.firstFormGroup.get('custInfo').get('prtnr_mntce_chrge_at').setValue(''); 
      this.read = false
    }
  ...
  Viewpoplst() {
    ...
    const rte = `caf/getPopByAgntId/${this.usrdtls.usr_ctgry_ky}`;
    this.crdsrv.get(rte).subscribe((res) => {
  ...
  getpopLst() {
    ...
    if (this.frm_actn == "update") {
      const rte = `caf/getPopByAgntId/${this.frmData.agnt_id}`;
      this.crdsrv.get(rte).subscribe((res) => {
        this.poplst = res['data'];
        console.log(res['data'])
        console.log(this.poplst)
...
//   }
  ...
  checkValue(event: any) {
    ...
    if (event.checked == true) {
      ...
      this.firstFormGroup.get('custInfo').get('blng_cntct_nm').setValue(data.hdOrgName);
  ...
  newentry() {
    ...
    // this.usrdtls = JSON.parse(localStorage.getItem('usrDtls'));
     const rte = "caf_operations/illprovision";
    // const rte = "caf/caf";
    ...
    data["instd_by_prntnr_id"] = this.firstFormGroup.value.custInfo.instd_by_prntnr_id,
    data["prtnr_mntce_chrge_at"] = this.firstFormGroup.value.custInfo.prtnr_mntce_chrge_at,
    data["lg_id"] = "lag::"
    ...
    data["mdle_cd"] = this.boxDetails[0].mdle_cd
    ...
    data["cst_at"] = this.entfrmData[0].total
    data["cre_srvc_id"] = this.entfrmData[0].cre_srvc_id
    data["crnt_pckge_id"] = this.entfrmData[0].ill_pckge_id
    ...
    let adjstdPrt = 0;
    if (data.olt_crd_nu == 1 ) {
      console.log("in if")
      adjstdPrt = data.olt_prt_nm + 8;
    }
    else{
      console.log("in else")
      adjstdPrt = data.olt_prt_nm
    }
    ...
    // data["aaa_cd"] = this.aaa_cd + ":" + data.olt_crd_nu + ":" + data.olt_prt_nm + ":" + this.onu_id;
      ...
      data["caf_type_id"] = 2
      data["prnt_cstmr_id"] = this.entfrmData[0].entfrmData.cstmr_id
      // data["hdOrgName"]=this.firstFormGroup.value.custInfo.hdOrgName,
    console.log(data)
