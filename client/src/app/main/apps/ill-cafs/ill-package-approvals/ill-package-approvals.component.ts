...
@Component({
  selector: 'app-ill-package-approvals',
  templateUrl: './ill-package-approvals.component.html',
  styleUrls: ['./ill-package-approvals.component.scss']
})
export class IllPackageApprovalsComponent implements OnInit {
  ...
  rcntPckgColumnDefs;
  rcntPckgAprvlLst;
  getHeaderDtls = function () { return { 'title': 'ILL Package Approvals Level-1', 'icon': 'money' }; };
  slctdrow: any;
  /**
  ...
  getRcntApprvlLst(){
    let rte = `caf/getRcntApprovalPckges/1`;
    this.crdSrvc.get(rte).subscribe(res=>{
      ...
      this.rcntPckgColumnDefs = [
        ...
        { headerName: 'Approved Date', format: false, field: 'lvl_1_apprv_ts', alignment: 'left', cellClass: 'pm-grid-number-cell', width: 150, height: 40, filter: true },
        { headerName: 'Approved Comment', format: false, field: 'lvl_1_apprv_desc_txt', alignment: 'left', cellClass: 'pm-grid-number-cell', width: 150, height: 40, filter: true },
  ...
  getLgnUsrUploads() {
    let rte = `caf/getMyApprovalPckges/1`;
    this.crdSrvc.get(rte).subscribe(res=>{
      console.log(res['data']);
  ...
  getPckgesApprvlLst(): void {
    ...
    const rte = `caf/getIllApprovalPckges`;
      ...
      if (response) {
        ...
        if (response === 'yes') {
          ...
          for (let i = 0; i < this.slctdrow.length; i++) {
            this.slctdrow[i]['lvl_1_apprv_desc_txt'] = this.pckgApprvlFormGroup.value.pckge_aprvl_desc_tx
          }
          let rte = `caf/ApprovalPckgeslevlOne`
          console.log(this.slctdrow)
