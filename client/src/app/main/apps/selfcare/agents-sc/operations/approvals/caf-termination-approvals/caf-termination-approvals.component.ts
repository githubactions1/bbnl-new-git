...
@Component({
  selector: 'app-caf-termination-approvals',
  templateUrl: './caf-termination-approvals.component.html',
  styleUrls: ['./caf-termination-approvals.component.scss']
})
export class CafTerminationApprovalsComponent implements OnInit {
  ...
  getcafsTrmndAprvlLst(): void {
    ...
    this.slctdCafs = [];
    this.loader = true
    const rte = `caf/terminate/cafs/approval`;
    this.crdSrv.get(rte).subscribe((res) => {
      if (res['status'] === 200) {
        ...
        this.cafTrmndAprvlColumnDefs = [
          ...
          { headerName: 'Approved By', field: 'fst_nm', alignment: 'left', cellClass: 'pm-grid-number-cell', width: 200, height: 40, filter: true, columnFiltering: false },
          { headerName: 'Approved ON', field: 'aprvd2_ts', alignment: 'left', cellClass: 'pm-grid-number-cell', width: 200, height: 40, filter: true, columnFiltering: false },
          { headerName: 'Approved Mobile No', field: 'mbl_nu', alignment: 'left', cellClass: 'pm-grid-number-cell', width: 200, height: 40, filter: true, columnFiltering: false },
          { headerName: 'Approval Comment', field: 'aprvl2_cmnt_tx', alignment: 'left', cellClass: 'pm-grid-number-cell', width: 200, height: 40, filter: true, columnFiltering: false },
        ];
      }
    });
  }
  ...
  getLgnUsrUploads(): void {
    ...
    this.slctdCafs = [];
    this.loader = true;
    const rte = `caf/terminate/cafs/approved/user`;
  ...
  getAllRcntUploads(): void {
    ...
    this.loader = true;
    const rte = `caf/terminate/cafs/approved/recent`;
      ...
      this.confirmMsgDialogRef.afterClosed().subscribe((response) => {
        if (response) {
          ...
          if (response === 'yes') {
            ...
            // return;
            this.shwLdr = true;
            const rte = `caf_operations/approve-terminate`;
        ...
        if (response) {
          ...
          if (response === 'yes') {
            ...
            for (let u = 0; u < this.slctdCafs.length; u++) {
              this.postSlctdCafsData.push({
                caf_id: this.slctdCafs[u].caf_id, caf_trmnd_aprvl_desc_tx: this.cafTrmndFormGroup.value.caf_trmnd_aprvl_desc_tx,
                trmnd: 'rjctd_in', rjct_cmntx_tx: 'rjctd_cmnt_tx'
              });
