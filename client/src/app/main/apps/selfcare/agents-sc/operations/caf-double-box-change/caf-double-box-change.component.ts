...
@Component({
  selector: 'app-caf-double-box-change',
  templateUrl: './caf-double-box-change.component.html',
  styleUrls: ['./caf-double-box-change.component.scss']
})
export class CafDoubleBoxChangeComponent implements OnInit {
  permissions;
  ...
  getHeaderDtls = function () { return { "title": "Double Box Change", "icon": "list_alt" } }
      ...
      this.crdsrv.create(data, rte).subscribe((res) => {
        ...
        if (res['status'] == 200) {
          this.columnDefs = [
            ...
            { headerName: 'CAF Number', field: 'caf_nu', alignment: "center", cellClass: "pm-grid-number-cell", width: 110, sortable: true, filter: true },
            // { headerName: 'LMO Code', field: 'agnt_cd', alignment: "center", cellClass: "pm-grid-number-cell", width: 115, sortable: true, filter: false },
            { headerName: 'Aadhar Number', alignment: "center", field: 'adhr_nu', cellClass: "pm-grid-number-cell", width: 120, sortable: true, filter: true },
            { headerName: 'Mobile Number', field: 'mbl_nu', alignment: "center", cellClass: "pm-grid-number-cell", width: 120, sortable: true, filter: true },
            { headerName: 'First Name', field: 'frst_nm', alignment: "left", cellClass: "pm-grid-number-cell", width: 125, sortable: true, filter: true },
            { headerName: 'Last Name', field: 'lst_nm', alignment: "left", cellClass: "pm-grid-number-cell", width: 135, sortable: true, filter: true },
            { headerName: 'Address 1', field: 'instl_addr1_tx', alignment: "left", cellClass: "pm-grid-number-cell", width: 115, sortable: true, filter: true },
            { headerName: 'Address 2', field: 'instl_addr2_tx', alignment: "left", cellClass: "pm-grid-number-cell", width: 135, sortable: true, filter: true },
            ...
            { headerName: 'IPTV Serial Number', alignment: "center", field: 'iptv_srl_nu', cellClass: "pm-grid-number-cell", width: 135, sortable: true, filter: true },
            { headerName: 'IPTV MAC Address', alignment: "center", field: 'iptv_mac_addr_tx', cellClass: "pm-grid-number-cell", width: 135, sortable: true, filter: true },
            { headerName: 'ONU Serial Number', alignment: "center", field: 'onu_srl_nu', cellClass: "pm-grid-number-cell", width: 135, sortable: true, filter: true },
            { headerName: 'ONU MAC Address', alignment: "center", field: 'onu_mac_addr_tx', cellClass: "pm-grid-number-cell", width: 125, sortable: true, filter: true },
            { headerName: 'Subscriber Code', alignment: "center", field: 'mdlwe_sbscr_id', cellClass: "pm-grid-number-cell", width: 120, sortable: true, filter: true },
            { headerName: 'Status', alignment: "center", field: 'sts_nm', cellClass: "pm-grid-number-cell", width: 80, sortable: true, filter: true },
  ...
  onBlur(){
    console.log(this.reason.length)
    console.log(this.iptvView)
    console.log(this.onuData.length)
    console.log(this.iptvData.length)
  }
  ...
  update() {
    ...
    var s = ((this.onuData && this.onuData.length >0) ? this.onuData[0].onu_mac_addr : "")
    var arr = s.split(':');
    var modified = ""
    ...
    let postData = {
      ...
      new_onu_srno: ((this.onuData && this.onuData.length >0) ? this.onuData[0].onu_srl_nu : ""),
      new_onu_mac_adrs: ((this.onuData && this.onuData.length >0) ? this.onuData[0].onu_mac_addr : ""),
      ...
      postData["iptv_stpbx_id"] = ((this.iptvData && this.iptvData.length > 0) ? this.iptvData[0].iptv_stpbx_id : "");
      postData["old_iptv_srno"] = this.selectedData.iptv_srl_nu
      postData["new_iptv_srno"] = ((this.iptvData && this.iptvData.length > 0) ? this.iptvData[0].iptv_srl_nu : "");
      postData["new_iptv_mac_adrs"] = ((this.iptvData && this.iptvData.length > 0) ? this.iptvData[0].iptv_mac_addr : "");
      postData["old_iptv_mac_adrs"] = this.selectedData.iptv_mac_addr_tx
      postData["onu_stpbx_id"] = ((this.onuData && this.onuData.length >0) ? this.onuData[0].onu_stpbx_id : "")
      ...
      if (this.iptvData && this.iptvData.length > 0  && this.onuData && this.onuData.length > 0) {
        console.log("step::::::::::11");
        postData['changed'] = 'both'
      } else if (this.iptvData && this.iptvData.length > 0) {
        console.log("step::::::::::22");
        postData['changed'] = 'iptv'
      } else if (this.onuData && this.onuData.length > 0) {
        postData['changed'] = 'onu'
        console.log("step::::::::::33");
      }
      if (postData["new_iptv_srno"] && postData["new_iptv_srno"].toLowerCase() == this.selectedData.iptv_srl_nu.toLowerCase() && postData.new_onu_srno.toLowerCase() == this.selectedData.onu_srl_nu.toLowerCase()) {
    ...
    if (this.onuData && this.onuData.length > 0 && this.onuData[0].onu_srl_nu.startsWith('DSNW')) {
      postData["serialNumber"] = this.onuData[0].onu_srl_nu.replace('DSNW', '44534E57');
    } else if (this.onuData  && this.onuData.length > 0 && this.onuData[0].onu_srl_nu.startsWith('ZTEG')) {
      postData["serialNumber"] = this.onuData[0].onu_srl_nu.replace('ZTEG', '5A544547');
    } else if (this.onuData  && this.onuData.length > 0 && this.onuData[0].onu_srl_nu.startsWith('YGE1')) {
      postData["serialNumber"] = this.onuData[0].onu_srl_nu.replace('YGE1', '59474531');
    } else if (this.onuData  && this.onuData.length > 0 && this.onuData[0].onu_srl_nu.startsWith('KONK')) {
    ...
    if (postData.changed == 'same') {
      ...
      let rte = 'caf_operations/double-box-change'
