...
@Component({
  ...
  styleUrls: ['./goi-customer.component.scss']
})
export class GoiCustomerComponent implements OnInit {
  ...
  shwPermMsg:string;
  disabled = false;
  getHeaderDtls = function (): any { return { 'title': 'GOI Customers  List', 'icon': 'people_outline' }; };
...
addNewCaf() {
  ...
  this.route.navigate([`/admin/caf/goi-new-caf`])
  ...
  getDetails() {
    ...
    this.columnDefs = [
      ...
      { headerName: 'Enterprise Type', field: 'entrpe_type_nm',  cellClass: 'pm-grid-number-cell', width: 125, filter: true, columnFiltering: true },
