...
@Component({
  selector: 'app-bbnl-customer',
  templateUrl: './bbnl-customer.component.html',
  styleUrls: ['./bbnl-customer.component.scss']
})
export class BbnlCustomerComponent implements OnInit {
  ...
  shwPermMsg:string;
  disabled = false;
  getHeaderDtls = function (): any { return { 'title': 'BBNL Customers  List', 'icon': 'people_outline' }; };
...
addNewCaf() {
  ...
  this.route.navigate([`/admin/caf/bbnl-new-caf`])
  ...
  getDetails() {
    ...
    this.cafFRm.value.CAf  ||  this.cafFRm.value.org_nm  || this.cafFRm.value.ent_dprtmnt_nm  ||
     this.cafFRm.value.Caf_sts  || this.cafFRm.value.adhar  || this.cafFRm.value.str_dt  || this.cafFRm.value.end_dt || this.cafFRm.value.till_dt  ){
        if (this.cafFRm.value['till_dt'] == true) {
          ...
          this.crdSrv.create(data, "caf/getbbnldt").subscribe(res => {
    ...
    this.columnDefs = [
      { headerName: 'Sno', field: 'sno', alignment: 'center', cellClass: 'pm-grid-number-cell', width: 100, filter: false, search: false, columnFiltering: false },
		...
		{ headerName: 'Project Name', field: 'prjct_nm', alignment: 'center', cellClass: 'pm-grid-number-cell', width: 100, height: 40, columnFiltering: false,  filter: true },
