...
exports.generateNewToken = (token, secretOrPrivateKey, ignoreExp) => {
  ...
  var verifyOptions = { audience: 'http://glits.com', subject: payload.mrcht_usr_id + '_user', ignoreExpiration: ignoreExp, algorithm: ["HS256"] };
  ...
  if (verified) {
    ...
    newToken = jwt.sign(payload, secretOrPrivateKey, { expiresIn: '3h', jwtid: '' + Math.floor(Math.random() * 20) });
